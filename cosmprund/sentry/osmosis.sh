...
PARENT_DIR="${HOME}/.sentry/.osmosisd"
SERVICE_NAME="sentry-osmosisd.service"
RPC_ADDRESS="http://localhost:20457"
CHAIN_NAME="osmosis"
...
if [[ "$CATCHING_UP" == "false" ]]; then
    ...
    PRUNE_OUTPUT=$(sudo docker run -v ${DATA_PATH}:${DATA_PATH} osmosis_cosmprund prune ${DATA_PATH} --cosmos-sdk=false 2>&1)
